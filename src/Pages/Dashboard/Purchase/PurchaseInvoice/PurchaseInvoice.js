...
import PurchaseReInvoiceDetails from "./PurchaseReInvoiceDetails";

const PurchaseInvoice = () => {
  
    const [filter, setFilter] = React.useState("");
    const [openNewPurchaseInvoice,setOpenNewPurchaseInvoice] = useState(false);
    const [openPurchaseInvoiceStatus,setOpenPurchaseInvoiceStatus] = useState(false);
    ...
    const backToList = () => {
      setOpenNewPurchaseInvoice(false);
      setOpenPurchaseInvoiceStatus(false);
    };

  return (
    <div>
      {openNewPurchaseInvoice ? <NewPurchaseInvoice backToList={backToList}/> 
      : openPurchaseInvoiceStatus 
      ? <PurchaseReInvoiceDetails backToList={backToList}/>
      ...
      <div className="package-text-field">
          ...
          <button type="button" className="btn create-new-btn" onClick={()=>(setOpenNewPurchaseInvoice(true))}>
            New Invoice
          </button>
        </div>
  ...
  onClick={() => {
    console.log("Invoice ID clicked!"); // Debugging log
    setOpenPurchaseInvoiceStatus(true);
  }}
>IN-075868</td>
                <td>22/03/2024</td>
                <td>PO-5325487</td>
