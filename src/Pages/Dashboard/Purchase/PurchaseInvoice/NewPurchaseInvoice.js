...
import AddCoulmnModal from "../../InventoryManagement/Salesorder/AddCoulmnModal";
import { paymentTerms } from "../../../../Data/SalesOrderData";
import PurchaseInvoiceDetails from "./PurchaseInvoiceDetails";

const NewPurchaseInvoice = ({ backToList }) => {
  const [rows, setRows] = useState([
    { name: "", quantity: 1, price: 0.0, discount: 0, gst: 0, amount: 0 },
  ]);
  const [addColumn, setAddColumn] = useState(false);
  const [newColumnName, setNewColumnName] = useState("");
  const [openPurchaseInvoiceDetails, setOpenPurchaseInvoiceDetails] = useState(false);
  ...
  const onSubmit = (data) => {
    setOpenPurchaseInvoiceDetails(true);
  };
  ...
  return (
    <div>
        {openPurchaseInvoiceDetails ? 
        <PurchaseInvoiceDetails backToList={()=>(setOpenPurchaseInvoiceDetails(false))}/>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="salesInvoice-form">
              <div className="new-package-name-sales mb-4 mt-5">
                <Form.Group className="col-md-4 customer-name-dropdown">
                  <div className="d-flex align-items-center gap-2">
                    <Form.Label className="custom-label mb-0">Vendor name</Form.Label>
                    {errors.customerName && (
              ...
              <div className="new-package-name-sales mb-4 mt-4">
                <div className="col-md-3 mt-3">
                  <Form.Group>
                    <div className="d-flex align-items-center">
                      <Form.Label className="custom-label mb-0">Due date</Form.Label>
                      {errors.dueDate && (
                        <ErrorOutlineOutlinedIcon className="text-danger ms-2" />
                      )}
                    </div>
                ...
                <div className="col-md-3 mt-3">
                  <Form.Group>
                    <div className="d-flex align-items-center">
                      <Form.Label className="custom-label mb-0">Order no</Form.Label>
                      {errors.orderNumber && (
                        <ErrorOutlineOutlinedIcon className="text-danger ms-2" />
                      )}
                    </div>
