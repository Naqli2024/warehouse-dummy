...
import { vendorPersonalInfoSchema } from "../../../Helper/validation";
import PersonalInfo from "../../../images/contact-info.svg";
...
const VendorPersonalInfo = ({ setActiveStep }) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const resultAction = await dispatch(createVendor(payload));

      if (createVendor.fulfilled.match(resultAction)) {
        const responseData = resultAction.payload;
        const vendorId = responseData?.vendor?._id;
        ...
        toast.error(resultAction.payload || "Failed to create vendor");
      }
    } catch (err) {
